        ...
        <tbody>
          ...
          <template v-for="(item, index) in cart.items" :key="index">
            <tr v-if="item">
              ...
              <td>
                <NuxtLink :to="`/newsite/product/${item.id}`">
                  <h6 :title="useTranslate(item.item, 'name')">
                    {{
                      item.item.nameEn.length > 20
                        ? useTranslate(item.item, "name").slice(0, 20) + "..."
                        : useTranslate(item.item, "name")
                    }}
                  </h6>
                </NuxtLink>
              </td>
              <td>
                <h6>
                  {{ item.item.note != "" ? item.item.note.slice(0, 20) : "" }}
                </h6>
              </td>
              <td>
                <font-awesome
          ...
          <tr>
            ...
            <td class="py-3">
              <strong class="mb-0"
                >{{ chippengcost }}
                {{ $i18n.locale === "ar" ? "درهم إماراتي" : "AED" }}</strong
              >
            </td>
            <!-- v-if="!shippingCostMap().value"  -->
            <!-- <td v-else class="py-3">
              <strong class="mb-0">{{ shippingCostMap().value }}    {{ $i18n.locale === "ar" ? "درهم إماراتي" : "AED" }}</strong>
            </td> -->
          </tr>
          <tr>
            <td class="py-3 text-center" colspan="1">
            ...
            <!-- <td v-else class="py-3">
              <strong class="mb-0"
                >{{ cart.totalPrice + shippingCostMap().value }}    {{ $i18n.locale === "ar" ? "درهم إماراتي" : "AED" }}</strong
              >
            </td> -->
          </tr>
        </tbody>
...
tbody tr:last-child {
  ...
  background:  linear-gradient(45deg, #146890, #45b2e9, #146890) !important;
