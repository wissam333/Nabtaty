      ...
      >
        ...
        <nav v-else class="d-flex flex-column justify-content-between">
          <ul class="d-flex" :class="locale === 'ar' ? 'ar-flex' : ''">
            <li v-for="menu in Menus?.items" :key="menu?.id" class="menu-item">
              <div class="paste-button" v-if="menu.englishName !== 'Products'">
                <div v-if="menu.type == 'Drop Down Menu'">
                  <nuxt-link
                    :to="`${menu.pageUrl}`"
              ...
              <div v-else class="paste-button">
                <div>
                  ...
                  <div
                    class="dropdown-content-products"
                    :class="locale === 'ar' ? 'ar' : ''"
                  >
                    <nuxt-link
                    ...
                    >
                      <img
                        class="cat-thumb"
                        :src="`${apiBase}${child.imageDataURL}`"
                        :alt="child.nameEn"
                      />
        ...
        >
          ...
          <div
            v-else
            class="pointer text-center dropdown"
            aria-current="page"
            aria-expanded="false"
            data-bs-toggle="dropdown"
          >
            <i class="fa fa-user-o acountlogo" aria-hidden="true"></i>
          </div>
          <ul v-if="isAuth.isAuthenticated" class="dropdown-menu background">
            <NuxtLink to="/my-account" class="">
              <li class="dropdown-item text-white">
                {{ $t("My Account") }}
              </li>
            </NuxtLink>
            <li class="dropdown-item" @click="logout">
              <NuxtLink to="/" class="text-white"> {{ $t("Logout") }}</NuxtLink>
            </li>
          </ul>
          <!-- cart -->
          <div class="cart mx-2 pointer">
            <img width="35" src="/photos/cart.png" alt="" />
          </div>
          <!-- search -->
          <div class="search mx-2 pointer">
            <img width="35" src="/photos/search.png" alt="" />
          </div>
        </div>
      </header>
    </div>
