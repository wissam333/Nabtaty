...
watchEffect(() => {
  // Iterate over each item and populate the children array based on parentId
  Menus.forEach((item) => {
    // Filter the items where parentId matches the item's id and assign them as children
    item.children = Menus.filter((child) => child.parentId === item.id);
  });
});
