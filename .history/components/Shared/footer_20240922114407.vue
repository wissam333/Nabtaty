...
watchEffect(() => {
  if (process.client) {
    if (ContactsDetails.value) {
      ...
      mobileData.value = details[0].replace("Mobile :", "").trim();
      emailData.value = details[1].replace("Email :", "").trim();
      websiteData.value = details[2].replace("Website :", "").trim();
    }
  }
});
