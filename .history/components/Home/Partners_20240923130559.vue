<template>
  <div>
    <div class="members">
      <h2 class="text-white mb-10 px-3">
        <img src="/4.png" alt="" class="me-2" width="70" />
        <span class="underline">
          {{ $i18n.locale === "ar" ? "الأعضاء" : "Members" }}
        </span>
      </h2>
      <div class="cont">
        <div class="slider">
                ...
                >
                  <div class="sponser-img">
                    <img
                      ...
                      :src="`${apiBase}/${member.image1}`"
                      v-if="member.image1"
                    />
                    <img
                      v-else
                      src="/logo/photo_5996792557864075100_x.jpg"
                      alt=""
                    />
                  </div>
...
const { data: Members, pending: loadingMembers } = await useGetSiteApi().GetAll(
  `${Blocks}?categoryId=4`,
  ["name", "englishName", "image1", "id", "blockPhotos", "url1", "parent"]
);
</script>
