<template>
  ...
  >
    <div class="container mt-10">
      <div class="see-all">
        <nuxt-link             :to="`/FoodByCategory/${useAllFoodesTypes().value[8]?.id}`" class="btn">
          {{ $i18n.locale === "ar" ? "رؤية الكل" : "See All" }}
        </nuxt-link>
      </div>
      <h1>
        <img class="mx-2" width="65" src="/photos/1-1.png" alt="" />
        <span>{{ $i18n.locale === "ar" ? "وجبة فطور وغداء" : "BRUNCH" }}</span>
      </h1>
      <!-- <h1>
          <span>
...
const { data: Products, pending } = await useGetSiteApi().GetAll(
  `${api.GetProducts}?CategoryId=${useAllFoodesTypes().value[8]?.id}&type=B2B`
);
</script>
