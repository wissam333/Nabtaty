      ...
      <div class="slider">
        ...
        <div v-else>
          ...
          <!-- <div class="album">
            <nuxt-link to="/Media/Photos"
              >Go To Photo Album
              <font-awesome class="mx-2" :icon="['fas', 'arrow-left']"
            /></nuxt-link>
          </div> -->
        </div>
      </div>
...
const fetchPhotos = async () => {
  ...
  try {
    ...
    const { data, error: fetchError } = await GetAll(
      `${Blocks}?categoryId=3&parentId=0`,
      ["id", "blockPhotos", "name", "englishName", "image1"]
    );
    if (fetchError.value) {
      errorPhotos.value = fetchError.value;
    } else {
      Photos.value = [
        ...data.value.items[0].blockPhotos,
        ...data.value.items[1].blockPhotos,
      ];
    }
  } catch (err) {
