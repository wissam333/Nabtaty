<template>
  ...
  >
    <div class="container mt-10">
      <div class="see-all">
        <nuxt-link :to="`/Products/11`" class="btn">
      ...
      <h1>
        ...
        <span>
          {{ $i18n.locale === "ar" ? "وصل حديثا" : "New Arrivals" }}
        </span>
      </h1>

      <div v-if="loading" class="text-center text-white loader">
...
const getProducts = async () => {
  loading.value = true;
  if (locale.value === "ar") {
    const { data: productsData, pending } = await useFetch(`${api.GetNewest}`, {
      credentials: "include",
      baseURL: apiBase,
      headers: {
        "accept-language": "ar",
