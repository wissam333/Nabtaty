...
const handleSubmit = async (values, actions) => {
  const AddPersonValue = {
    ...
    birthDate: date.value ? convertDate(date.value) : null,
  ...
  const { data: AddPersonData } = await useFetch(`${api.PersonClientsApi}`, {
    baseURL: apiBase,
    method: "POST",
    body: AddPersonValue,
    headers: {
  ...
  if (AddPersonData.value) {
    if (AddPersonData.value.hasErrors) {
      ...
      const { data: DataUserInfo } = await useGetSiteApi().GetById(
        `${api.PersonClientsApi}/`,
        userInfo.id.toString()
      );
      localStorage.setItem("userInfo", JSON.stringify(DataUserInfo.value.data));
      useUserInfo().value = DataUserInfo.value.data;
