<template>
  <div class="" id="table">
    <div class="table-content table-responsive" v-if="order">
      <table class="table" ref="DownloadComp">
        ...
        <tbody>
          <tr v-for="(order, index) in order" :key="index">
            ...
            <td v-if="order.princedomId && order?.section?.nameAr">
              <!-- {{ $i18n.locale == 'en' ?  order.restuarant.nameEn : order.restuarant.nameAr }} -->
              {{
                (getCityNameById(order.princedomId) ?? "----") +
                (locale === "ar"
                  ? order?.section?.nameAr ?? "----"
                  : order?.section?.nameEn ?? "----")
              }}
            </td>
            <!-- <td v-else-if="order.address">
                              {{ order.address ?? '----' }}
            ...
            <td v-else>
              {{
                order.cityName ??
                ($i18n.locale === "ar" ? "يتم التحميل..." : "Loading...")
              }}
            </td>
            <td>
              {{ order.totalPrice ?? "----" }}
              {{ useTranslate(order.currency, "name") }}
            </td>
            <td>
              {{ order.shippingCost }}
              {{ useTranslate(order.currency, "name") }}
            ...
            <td @click="detail(order)">
              <span style="cursor: pointer">
                <font-awesome :icon="['fas', 'link']" color="#00c2ec" />
              </span>
            </td>
          </tr>
        </tbody>
          ...
          <ul class="timeline">
            <li
              :class="
                ...
                finalOrdinaryOrderTrackings?.shippingStatus === 'Shipping' ||
...
const getCityNameById = (id) => {
  ...
  return (princedomName ? princedomName : "----") + " - ";
};
...
.table-responsive {
  ...
  // @include horizontal-scrollbar(6px, '#2e2b2b', $primary, '#555');
}
