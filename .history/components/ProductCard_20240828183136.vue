<template>
  ...
  <div class="product-info">
    ...
    <nuxt-link :to="`/Products/${product.id.toString()}`" class="btn btn-main">
      <span>{{ $i18n.locale === "ar" ? "اطلب الآن" : "Order Now" }}</span>
    </nuxt-link>

    <p class="price">
      <span :class="product?.productOffer.newPrice ? 'line-through' : ''">
        {{ product?.price }}
        {{ $i18n.locale === "ar" ? "AED" : "AED" }}
      </span>
    </p>
    <p class="old-price" v-if="product?.productOffer.newPrice">
