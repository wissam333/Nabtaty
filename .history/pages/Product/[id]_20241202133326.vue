    ...
    <div v-else class="container" style="position: relative">
      ...
      <div class="row reverse-mobile">
        ...
        <div class="info col-lg-6">
          <!-- Product Fav -->
          <div class="heart">
            <span v-if="useUserInfo().value?.email">
              <font-awesome
                v-if="!isFavorite"
                @click="AddToFavorite()"
                :icon="['far', 'heart']"
          ...
          <h2 class="price">
            <div class="mb-4" v-if="Product[0]?.price1 > 0">
              <span>{{ $i18n.locale === "ar" ? "كبير جدا" : "XL" }} : </span>
              {{ Product[0]?.price1 }}
              {{ $i18n.locale === "ar" ? "دينار" : "JOD" }}
            </div>

            <div class="mb-4" v-if="Product[0]?.price2 > 0">
              <span>{{ $i18n.locale === "ar" ? "كبير" : "L" }} : </span>
              {{ Product[0]?.price2 }}
              {{ $i18n.locale === "ar" ? "دينار" : "JOD" }}
            </div>

            <div class="mb-4" v-if="Product[0]?.price3 > 0">
              <span>{{ $i18n.locale === "ar" ? "متوسط" : "M" }} : </span>
              {{ Product[0]?.price3 }}
              {{ $i18n.locale === "ar" ? "دينار" : "JOD" }}
            </div>

            <div class="mb-4" v-if="Product[0]?.price4 > 0">
              <span>{{ $i18n.locale === "ar" ? "صغير" : "SM" }} : </span>
              {{ Product[0]?.price4 }}
              {{ $i18n.locale === "ar" ? "دينار" : "JOD" }}
            </div>
          </h2>
          ...
          <div class="d-flex align-items-center">
            ...
            <!-- Add to cart button -->
            <div class="add-to-cart mt-14 mx-3" @click="AddToCart(Product[0])">
              <button class="btn">
                <span>
              ...
              >
                ...
                <v-window-item value="three" class="">
                  <h5 style="color: #78c044; font-weight: bold">
                    <font-awesome :icon="['fas', 'hands-holding']" />
                    {{
                      $i18n.locale == "en" ? "Ease of care" : "سهولة العناية"
                    }}
                  </h5>
                  <p>
                    {{ Product[0].medium }}
                ...
                <v-window-item value="six" class="">
                  <h5 style="color: #78c044; font-weight: bold">
                    <font-awesome :icon="['fas', 'temperature-high']" />
                    {{ $i18n.locale == "en" ? "Temperature" : "الحرارة" }}
                  </h5>
                  <p>
                ...
                <v-window-item value="seven" class="">
                  <h5 style="color: #78c044; font-weight: bold">
                    <font-awesome :icon="['fas', 'square']" />
                    {{ $i18n.locale == "en" ? "Pot" : "اصيص" }}
                  </h5>
                  <p>
                    {{ Product[0].pot }}
                  </p>
...
watchEffect(() => {
  if (process.client) {
    if (Product.value) {
      ...
      sizes.value = Product.value[0].size.split(",");
    }
  }
});
...
const AddToCart = (item) => {
  ...
  item.selectedSize = activeSize.value.toString();
  // return
...
useUserInfo().value?.email ? isInFavorite() : "";
...
const removeFromFavorite = async () => {
  ...
  let id = data.value.filter((e) => e.plantId == route.params.id);
