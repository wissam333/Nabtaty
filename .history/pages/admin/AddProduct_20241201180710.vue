...
let activeSize = ref(["M"]);
const activeSizesFunc = (size) => {
  if (activeSize.value.includes(size)) {
    activeSize.value = activeSize.value.filter((s) => s !== size);
  } else {
    activeSize.value.push(size);
  }
};
</script>
