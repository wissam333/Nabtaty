    ...
    <div class="table-responsive">
      ...
      <div v-else class="cat-tabel">
        <table>
          ...
          <tbody>
            <tr class="rows" v-for="cat in About?.data" :key="cat.id">
              <td class="truncate">
                <div>
                  <span>
                    <img
              ...
              <td class="">
                <div>
                  <Checkbox v-model="isActive" binary />
                </div>
              </td>
    ...
    <template #container="{ closeCallback }">
      <div class="dialog" :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
        <h5 class="fw-bold mb-5">
          {{ $i18n.locale === "ar" ? "إضافة لوغو جديد" : "Add New Logo" }}
        </h5>
        <div class="content">
...
const getInfo = async () => {
  loading.value = true;
  if (locale.value === "ar") {
    const { data: productsData, pending } = await useFetch(`${api.GetLogo}`, {
      baseURL: apiBase,
      credentials: "include",
      headers: {
        "accept-language": "ar",
...
const addInfo = async () => {
  ...
  const { data } = await useFetch(api.AddLogo, {
    baseURL: apiBase,
    credentials: "include",
    method: "POST",
    body: formData,
...
const editfunc = (image) => {
  editsrc.value = apiBase + "/" + image.photoPath;
};
let editedCat = ref();
const editCat = async () => {
...
const isActive = computed({
  get: () => !!props.cat.is_active, // Convert to boolean
  set: (value) => (props.cat.is_active = value ? 1 : 0), // Convert back to 1/0
});
</script>
