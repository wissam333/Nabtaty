    ...
    <template #container="{ closeCallback }">
      <div class="dialog" :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
          ...
          <div>
            <div class="d-flex flex-column-reverse mb-3">
              <input
                ...
                autocomplete="off"
                :placeholder="
                  $i18n.locale === 'ar' ? 'الاسم بالعربي' : 'Arabic Name'
                "
            ...
            <div class="d-flex flex-column-reverse mb-3">
              <input
                ...
                autocomplete="off"
                :placeholder="
                  $i18n.locale === 'ar' ? 'الاسم بالانجليزي' : 'English Name'
                "
              />
              <label for="nameEn">{{
                $i18n.locale === "ar" ? "الاسم بالانجليزي" : "English Name"
        ...
        <div class="mt-5 d-flex w-100 justify-content-end">
          ...
          <button class="btn btn-success mx-2" @click="addLocation" autofocus>
            {{ $t("Add") }}
          </button>
        </div>
      </div>
    ...
    <template #container="{ closeCallback }">
      <div class="dialog" :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
        ...
        <div class="mt-5 d-flex w-100 justify-content-end">
          ...
          <button class="btn btn-success mx-2" @click="editLocation" autofocus>
            {{ $t("Edit") }}
          </button>
        </div>
      </div>
...
const addLocation = async () => {
  visible.value = false;
  const { AddProduct, error } = await useFetch(api.AddLocation, {
    baseURL: apiBase,
    method: "POST",
    credentials: "include",
    body: {
  ...
  //message
  locale.value === "en"
    ? $awn.success("Location Added Successfully", {
        durations: { global: 5000 },
      })
...
const editfunc = async () => {
  const { data: ar } = await useFetch(
    `${api.GetLocationById}/${editedCat.value.id}`,
    {
      credentials: "include",
      baseURL: apiBase,
      headers: {
  ...
  editnameAr.value = ar.value.name;
  editnameEn.value = en.value.name;
  editPrice.value = en.value.price;
};
...
const editLocation = async () => {
  ...
  //message
  locale.value === "en"
    ? $awn.success("Location Edited Successfully", {
        durations: { global: 5000 },
      })
