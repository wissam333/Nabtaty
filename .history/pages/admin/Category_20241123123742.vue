    ...
    <div class="table-responsive">
      ...
      <div v-else class="cat-tabel">
        <table>
          <thead>
            <tr>
              <th>{{ $i18n.locale === "ar" ? "الترتيب" : "Priority" }}</th>
          ...
          <div>
            <div class="d-flex flex-column-reverse mb-3">
              <input
                ...
                type="number"
                autocomplete="off"
                :placeholder="$i18n.locale === 'ar' ? 'الترتيب' : 'Priority'"
                required
              />
              <label for="priority">{{
                $i18n.locale === "ar" ? "الترتيب" : "Priority"
              }}</label>
            </div>
...
const getCats = async () => {
  ...
  if (locale.value === "ar") {
    ...
    watchEffect(() => {
      if (process.client) {
        if (productsData.value) {
          cats.value = productsData.value.sort(
...
const addCat = async () => {
  ...
  const { AddProduct, error } = await useFetch(api.AddCat, {
    baseURL: apiBase,
    method: "POST",
    credentials: "include",
    body: formData,
...
const editfunc = async () => {
  const { data: ar } = await useFetch(
    `${api.GetCatById}/${editedCat.value.id}`,
    {
      credentials: "include",
      baseURL: apiBase,
      headers: {
  ...
  editPriority.value = en.value.priority;
};
