  ...
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    <div class="title-container">
      ...
      <div class="options">
        ...
        <div
          v-if="!details"
          class="reload btn btn-success mx-2"
          @click="getInfo()"
        >
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>{{ $i18n.locale === "ar" ? "تفاصيل" : "Details" }}</th>
            </tr>
          </thead>
          <tbody>
            <tr class="rows" v-for="product in Orders" :key="product.cartId">
              <td class="fw-bold">{{ product.cartId }}</td>
              <td class="fw-bold">
                {{ getCustomerName(product.customer_id) }}
              </td>
              <td>
                <p class="badge rounded-pill bg-primary me-1">
                  JOD {{ product.total_price }}
                </p>
              </td>
              ...
              <td class="fw-bold edit-link" @click="getDetails(product.cartId)">
        ...
        <table>
          <thead>
            <tr>
              <th>{{ $i18n.locale === "ar" ? "الاسم" : "Name" }}</th>
              <th>
                {{ $i18n.locale === "ar" ? "الحجم" : "Size" }}
              </th>
              <th>{{ $i18n.locale === "ar" ? "الكمية" : "Quantity" }}</th>

              <th>
                {{ $i18n.locale === "ar" ? "السعر الإفرادي" : "Single Price" }}
              </th>
              <th>
          ...
          <tbody>
            <tr
              class="rows"
              v-for="product in OrderDetails?.items"
              :key="product.id"
            >
              <td class="fw-bold">
                {{ product.plantName }}
              ...
              <td class="fw-bold">JOD {{ Number(product?.plantPrice) }}</td>
              <td class="fw-bold">
                JOD {{ Number(product?.plantPrice) * product.quantity }}
              </td>
            </tr>
          </tbody>
        </table>
...
const getDetails = async (id) => {
  details.value = true;
  loading.value = true;
  const { data: DataOrderDetails, pending } = await useFetch(
  ...
  OrderDetails.value = DataOrderDetails.value;
  loading.value = false;
};
</script>
