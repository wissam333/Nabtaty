          ...
          <tbody>
            <tr class="rows" v-for="cat in About" :key="cat.id">
              <td>
                <div>
                  <span>{{
                    cat.content?.length > 30
                      ? cat.content.slice(0, 30) + "..."
                      : cat.content
                  }}</span>
                </div>
              </td>
...
const addInfo = async () => {
  const { data } = await useFetch(api.AddPrivacy, {
    baseURL: apiBase,
    credentials: "include",
    method: "POST",
    body: {
...
const editfunc = async (info) => {
  const { data: ar } = await useFetch(
    `${api.GetPrivacyById}/${editedCat.value.id}`,
    {
      credentials: "include",
      baseURL: apiBase,
      headers: {
  ...
  editDescription_AR.value = ar.value.content;
  editDescription_EN.value = en.value.content;
};
