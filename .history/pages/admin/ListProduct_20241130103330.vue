<template>
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    <div class="title-container">
      ...
      <div class="options">
        ...
        <div class="reload btn btn-success mx-1" @click="getProduct()">
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>
                {{ $i18n.locale === "ar" ? "الفئة الفرعية" : "Sub Category" }}
              </th>
              <th>{{ $i18n.locale === "ar" ? "جديد" : "New" }}</th>
              <th>{{ $i18n.locale === "ar" ? "مقترح" : "Recommended" }}</th>
              <th>{{ $i18n.locale === "ar" ? "فعال" : "Active" }}</th>
          ...
          <tbody>
            <tr class="rows" v-for="product in filteredArray" :key="product.id">
              ...
              <td class="fw-bold">JOD {{ product.price }}</td>
              <td>
                <p class="badge rounded-pill bg-primary me-1">
                  {{ subcat(product.category_id) }}
                </p>
              </td>
              <td>
                <p class="badge rounded-pill bg-secondary me-1">
                  {{ subSubcat(product.subcategory_id) }}
                </p>
              </td>
              <td>
                <p
              ...
              <td class="">
                <div v-if="product.is_active" @click="editActive(product)">
                  <Checkbox :modelValue="isActive(product.is_active)" binary />
                </div>
                <div v-else @click="editActive(product)">
                  <Checkbox :modelValue="isActive(product.is_active)" binary />
                </div>
              </td>
              <td>
  ...
  <ElementsAppModal :isOpen="deleteModal" @updateIsOpen="updateDeleteModal">
    ...
    <template #footer>
      ...
      <button class="btn btn-danger" @click="deletePlant()">
        {{ $t("Delete") }}
      </button>
    </template>
  </ElementsAppModal>
...
const editActive = async (product) => {
  const formData = new FormData();

  formData.append("is_active", titleEn.value);

  const { AddProduct, error } = await useFetch(
    `${api.UpdatePlant}/${product.id}`,
    {
      credentials: "include",
      baseURL: apiBase,
      method: "PUT",
...
const getSubCats = async () => {
  if (locale.value === "ar") {
    const { data: productsData, pending } = await useFetch(`${api.GetSubCat}`, {
      credentials: "include",
      baseURL: apiBase,
      headers: {
        "accept-language": "ar",
    ...
    watchEffect(() => {
      if (process.client) {
        if (productsData.value) {
          Subcats.value = productsData.value;
        }
      }
    });
  } else {
...
const subcat = (id) => {
  let subcatName = cats.value?.filter((sub) => sub.id === id);
  if (subcatName) {
    return subcatName[0]?.category_name;
  }
};

const subSubcat = (id) => {
  let subcatName = Subcats.value?.filter((sub) => sub.id === id);
  if (subcatName) {
    return subcatName[0]?.subcategory_name;
  }
};
...
const deletePlant = async () => {
  ...
  // delete plant photos
  await data.value[0].photos?.map(async (photo) => {
...
const exportToExcel = () => {
  ...
  let data;
  if (locale.value === "en") {
    data = filteredArray.value.map((product) => ({
      Name: product.plant_name,
      Price: product.price,
      Categories: subcat(product.category_id),
      Sub_Category: subSubcat(product.subcategory_id),
      New: product.newest ? "Yes" : "No",
      Recommended: product.recommended ? "Yes" : "No",
    }));
  } else {
    data = filteredArray.value.map((product) => ({
      الاسم: product.plant_name,
      السعر: product.price,
      الفئة: subcat(product.category_id),
      الفئة_الفرعية: subSubcat(product.subcategory_id),
      جديد: product.newest ? "نعم" : "لا",
      مقترح: product.recommended ? "نعم" : "لا",
    }));
  }
