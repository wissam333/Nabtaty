  ...
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    <div class="title-container">
      ...
      <div class="options">
        ...
        <div class="reload btn btn-success" @click="getCats()">
    ...
    <div class="table-responsive">
      ...
      <div v-else class="cat-tabel">
        <table>
          ...
          <tbody>
            <tr class="rows" v-for="cat in cats" :key="cat.id">
              <td class="d-flex align-items-center">
                <div class="cat-img mx-2">
                  <img
                    v-if="cat.photo"
                    :src="apiBase + '/' + cat.photo"
                    alt=""
                  />
                  <img
                    v-else
    ...
    watchEffect(() => {
      if (process.client) {
        if (productsData.value) {
          MainCats.value = productsData.value;
        }
      }
    });
  } else {
...
const DeleteCat = async () => {
  ...
  //message
  locale.value === "en"
    ? $awn.success("Category Deleted Successfully", {
...
const addCat = async () => {
  ...
  const { AddProduct, error } = await useFetch(api.AddSubCat, {
    baseURL: apiBase,
    method: "POST",
    credentials: "include",
    body: formData,
...
const editfunc = async () => {
  const { data: ar } = await useFetch(
    `${api.GetSubById}/${editSelectedMainCat.value.id}`,
    {
      credentials: "include",
      baseURL: apiBase,
      headers: {
  ...
  const { data: en } = await useFetch(
    `${api.GetSubById}/${editedCat.value.id}`,
    {
      credentials: "include",
      baseURL: apiBase,
      headers: {
  ...
  editsrcFile.value = apiBase + "/" + en.value.photo;
