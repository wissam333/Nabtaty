              ...
              >
                <template #item="{ item, hasSubmenu }">
                  <div class="filterbox pointer d-flex justify-content-between">
                    <span
                      class="cat"
                      @click="setselectedId(item, hasSubmenu)"
...
const { data: SubCatForMainCat } = await useGetSiteApi().GetAll(
  `${api.GetSubById}/${route.params.id}`
);
watchEffect(() => {
  if (process.client) {
    if (SubCatForMainCat.value) {
      SubCatForMainCat.value.map(async (sub) => {
        const { data: data } = await useGetSiteApi().GetAll(
          `${api.GetSubCat}/${sub.id}`
        );
      });
    }
  }
...
const setselectedId = async (item, hasSubmenu) => {
  if (!hasSubmenu) {
    loading.value = true;
