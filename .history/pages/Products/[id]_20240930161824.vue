            ...
            >
              <template #item="{ item, hasSubmenu }">
                ...
                >
                  ...
                  <!-- <PanelMenu :model="AllCategories" class="w-full md:w-80">
                    <template #item="{ item, hasSubmenu }">
                      <p
                        class="filterbox pointer d-flex justify-content-between"
                        @click="setselectedId(item)"
                        :class="selectedId == item.id ? 'active' : ''"
                        v-if="item.parentCategoryId !== null"
                      >
                        {{
                          $i18n.locale == "en"
                            ? item.englishName
...
// get all categories
const { data: AllCategories } = await useGetSiteApi().GetAll(
...
watchEffect(() => {
  if (process.client) {
    if (Array.isArray(AllCategories.value)) {  // Check if AllCategories.value is an array
      let AllCategoriesArr = ref(AllCategories.value); // Directly use the array

      AllCats.value = AllCategoriesArr.value
        .filter((cat) => cat.parentCategoryId === null) // Get top-level categories
        .map((parent) => {
          const items = AllCategoriesArr.value.filter(
            (cat) => cat.parentCategoryId === parent.id
          );
          
          // Return the parent with a new "items" field containing its child categories
          return { ...parent, items };
        });
    } else {
      console.log("AllCategories is not an array:", AllCategories.value); // Debug log
    }
  }
});
