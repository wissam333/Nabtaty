<template>
  <div class="news-detail-page nav-margin">
    <div class="container">
      ...
      <div v-else-if="loadingNews === false" class="container">
        <h2 class="news-title mb-10 d-flex align-items-start">
          <img src="/photos/7-1.png" width="60" alt="" />
          <div>
            {{ $i18n.locale === "ar" ? News?.name : News?.englishName }}
            <p class="date">{{ formatDate(News?.startDate) }}</p>
          </div>
        </h2>
        ...
        <div v-if="News?.description1" class="news-detail-wrapper row mb-5">
          ...
          >
            <img
              ...
              class="main-img"
            />
            <img v-else src="/photos/7-1.png" alt="" class="main-img" />
          </div>
...
const fetchNews = async () => {
  ...
  try {
    const { GetById } = await useGetSiteApi();
    const { data, error: fetchError } = await GetById(
      `${Blocks}/`,
      [
        "id",
        "image1",
