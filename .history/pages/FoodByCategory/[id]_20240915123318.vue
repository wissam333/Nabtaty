...
const { data: Food, pending } = await useGetSiteApi().GetAll(
  `${api.GetProducts}?CategoryId=${route.params.id}`
);
...
const setselectedId = async (item) => {
  ...
  const { data: ProductsByCat, pending } = await useGetSiteApi().GetAll(
    `${api.GetProducts}?CategoryId=${item.id}?pageNumber=${
      page.value
    }&${Date.now()}`
  );
...
const loadMore = async () => {
  ...
  if (selectedId.value) {
    const { data: ProductsByCat, pending } = await useGetSiteApi().GetAll(
      `${api.GetProducts}?CategoryId=${selectedId.value}&pageNumber=${page.value}`
    );
