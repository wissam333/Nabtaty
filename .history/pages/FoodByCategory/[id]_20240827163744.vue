    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-lg-10">
          ...
          <div v-else class="row" v-show="Products?.data?.length > 0">
            <div
              :class="[
          ...
          <div class="load-more">
            <button
              @click="loadMore()"
              class="btn"
              v-if="hasMore || !pending || !loading"
            >
              {{ $i18n.locale == "en" ? "Load more" : "تحميل المزيد" }}
            </button>
          </div>
          <h1
            v-show="Products?.data?.length === 0 && !pending && !loading"
...
hasMore.value = await Products.value.data.hasNextPage;
...
const setselectedId = async (item) => {
  ...
  const { data: ProductsByCat, pending } = await useGetSiteApi().GetAll(
    `${api.GetProducts}?CategoryId=${item.id}&type=B2B/`
  );
  Products.value = ProductsByCat.value;
  hasMore.value = ProductsByCat.value.data.hasNextPage;
  loading.value = false;
};
...
const loadMore = async () => {
  ...
  if (selectedId.value) {
    ...
    Products.value.push(ProductsByCat.value.data);
    hasMore.value = ProductsByCat.value.data.hasNextPage;
    loading.value = false;
  } else {
    const { data: ProductsByCat, pending } = await useGetSiteApi().GetAll(
