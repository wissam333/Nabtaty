...
const has_mor = ref(false);
const page = ref(null);
page.value = 1;

route.query && route.query.id ? (selectedId.value = route.query.id) : "";

// Get Food
if (selectedId.value) {
  const { data: food } = await useFetch(`${api.GetFoods}`, {
    baseURL: apiBase,
    query: {
      companyId: 10,
  ...
  watchEffect(() => {
    if (process.client) {
      if (food.value) {
        food.value.data.forEach((item) => {
          item.qty = 1;
        });
        Foods.value = food.value.data;
        has_mor.value = food.value.hasNextPage;
      }
    }
  });
} else {
  const { data: food } = await useFetch(`${api.GetFoods}`, {
    baseURL: apiBase,
    query: { companyId: 10, pageNumber: page.value, orderBy: "Id descending" },
    headers: {
      Authorization: `Bearer ${useMainToken().value}`,
    },
  });
...
const loadmore = async () => {
  ...
  if (selectedId.value) {
    ...
    Foods.value = Foods.value.concat(food.value.data);
    has_mor.value = food.value.hasNextPage;
