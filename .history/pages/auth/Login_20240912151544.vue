          ...
          <div class="login">
            ...
            <div class="company">
              ...
              <p>
                {{
                  $i18n.locale === "ar"
                    ? "تم تصميم منصتنا مع أخذ التصميم الاحترافي في الاعتبار، وتوفر تجربة مخصصة. قم بالتسجيل للحصول على حساب للحجز والشراء عبر الإنترنت بسهولة، والتحقق من أسعار المنتجات والمخزون، وطلب العينات، وإنشاء القوائم وإدارتها. كل ما تحتاجه للحفاظ على مشاريعك على المسار الصحيح، كل ذلك في مكان واحد."
                    : "Designed with design professional in mind, our platform offers a tailored experince. Sign up for an account to easily reserve and purchase online, check product pricing and inventory, request samples, create and manage lists. Everything you need to keep your projects on track, all in one place."
                }}
              </p>
              <button class="btn btn-special">
                {{
    ...
    if (data.value.data.status == "Accepted") {
      ...
      if (infoData.value.succeeded) {
        ...
        $awn.success(
          locale.value === "ar"
            ? ` اهلاً بك ${
                infoData.value.data.fullNameAr
                  ? infoData.value.data.fullNameAr
                  : infoData.value.data.fullNameEn
              }`
            : `Welcome ${
                infoData.value.data.fullNameEn
                  ? infoData.value.data.fullNameEn
...
const schema = object({
  ...
  //   .required(
  //     locale.value === "ar"
  //       ? "رقم الهاتف او البريد الإلكتروني مطلوب"
