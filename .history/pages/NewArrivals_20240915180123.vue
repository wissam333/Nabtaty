  ...
  <div class="container nav-margin">
    <div class="text-center d-flex justify-center">
      <h1 class="">
        ...
        <span> {{ $i18n.locale == "ar" ? "وصل حديثاً" : "New Arrivals" }}</span>
      </h1>
    </div>
    <div
      v-if="pending"
      ...
      <div v-if="Special">
        <div class="row" v-if="Special.data.length > 0">
          ...
          <div
            ...
            :key="item.id"
            class="mb-4 col-6 col-sm-6 col-md-4 col-lg-4"
          >
            <ProductCard2
              class="mt-8"
              :id="item.id"
...
const {
  ...
  error,
  pending,
} = await useFetch(`${api.ProductdetailsApi}?isRecent=true`, {
  baseURL: apiBase,
  headers: {
...
const loadMore = async () => {
  ...
  const { data: Products, pending } = await useGetSiteApi().GetAll(
    `${api.ProductdetailsApi}?isRecent=true&pageNumber=${page.value}`
  );
