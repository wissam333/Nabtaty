  ...
  <div class="contain" :class="useSidebar().value ? 'shrink' : ''">
    <div class="title-container">
      <div class="head-title">
        <h4 class="header4">
          {{ $i18n.locale === "ar" ? "الرسائل" : "Messages" }}
        </h4>
      </div>
      <div class="options">
    ...
    <div class="table-responsive">
      ...
      <div v-else class="cat-tabel">
        <table>
          ...
          <tbody>
            <tr class="rows" v-for="cat in Messages?.data" :key="cat.id">
              <td>
                <div>
                  <span>{{ cat.name }}</span>
                </div>
              ...
              >
                <div>
                  <span>{{
                    cat.message.length > 20
                      ? cat.message.slice(0, 20) + "..."
                      : cat.message
                  }}</span>
                </div>
              </td>
              <td>
                <div>
                  <span>{{ cat.created_at.slice(0, 10) }}</span>
                </div>
              </td>
              <!-- <td>
                  <span
    ...
    <template #container="{ closeCallback }">
      <div class="dialog" :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
        <p class="fw-bold mb-5">
          {{ $i18n.locale === "ar" ? "تفاصيل الرسالة" : "Message Information" }}
        </p>
        <div class="content" style="direction: initial">
          {{ message }}
        </div>
          ...
          <div>
            <div class="d-flex flex-column-reverse mb-3">
              <input
                ...
                type="text"
                autocomplete="off"
                :placeholder="$i18n.locale === 'ar' ? 'الهاتف' : 'phone'"
                required
              />
              <label for="editPhone">{{
                $i18n.locale === "ar" ? "الهاتف" : "phone"
              }}</label>
            </div>
            ...
            <div class="d-flex flex-column-reverse mb-3">
              <input
                ...
                type="text"
                autocomplete="off"
                :placeholder="$i18n.locale === 'ar' ? 'الموقع' : 'Website'"
                required
                title="Enter only letters and spaces"
              />
              <label for="editWebsite">{{
                $i18n.locale === "ar" ? "الموقع" : "Website"
              }}</label>
            </div>
          </div>
        </div>
    ...
    watchEffect(() => {
      if (process.client) {
        if (productsData.value) {
          Messages.value = productsData.value;
        }
      }
    });
  } else {
...
const addInfo = async () => {
  const { data } = await useFetch(api.AddMessagesInfo, {
    baseURL: apiBase,
    credentials: "include",
    method: "POST",
    body: {
...
const editfunc = async (info) => {
  editPhone.value = info.phone;
  editEmail.value = info.email;
